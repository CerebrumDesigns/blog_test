...
class AddUser(db.Model, UserMixin):
...
class LoginForm(FlaskForm):
    username = StringField(label='Username', validators=[Email("Please enter a valid username")])
...
def add_user():
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            ...
            user = AddUser(name=form.name.data, email=form.email.data, fav_color=form.fav_color.data,
    ...
    our_users = AddUser.query.order_by(AddUser.id)
...
def update(id):
    form = UserForm()
    name_to_update = AddUser.query.get_or_404(id)
...
def delete(id):
    name_to_delete = AddUser.query.get_or_404(id)
    name = None
    form = UserForm()
